...
/**
 * A class that can calculate the vertex positions of an ocean-like plane. Does
 * not provide a mesh
 * 
 * @author sascha
 *
 */
class ComplexOceanEngine implements OceanEngine {
	...
	public void initVertices() {
		...
		for (int m_prime = 0; m_prime < Nplus1; m_prime++) {
			for (int n_prime = 0; n_prime < Nplus1; n_prime++) {
				...
				//Note the above are evaluatingot ~0, is it lack of precision?
//				System.out.println(htilde0._a);
//				System.out.println(htilde0._b);
	...
	public float phillips(int n_prime, int m_prime) {
		Vector2f k = new Vector2f((float) FastMath.PI * (2 * n_prime - N) / length,
				(float) FastMath.PI * (2 * m_prime - N) / length);
		...
		float k_dot_w = Maths.getNewNormalizedVector(k).dot(Maths.getNewNormalizedVector(windVector));
		float k_dot_w2 = k_dot_w * k_dot_w;

		float windLength = windVector.length();
		//System.out.println("Windlength: " + Float.toString(windLength));
...
//		System.out.println("------------------------------------------------------");
//		System.out.println("K_length2 * L2   " + Float.toString((k_length2 * L2)));
//		System.out.println("Wind Caused      " + Double.toString(FastMath.exp(-1.0f / (k_length2 * L2))));
//		System.out.println("K4     " + Double.toString(k_length4));
//		System.out.println("K dot W2     " + Double.toString(k_dot_w2));
//		System.out.println("DAmpening      " + Double.toString(FastMath.exp(-k_length2 * l2)));
	...
	public Complex hTilde_0(int n_prime, int m_prime) {
		...
		//guassian's good 
//		System.out.println("Random Gaussian - " + Float.toString(r._a));
//		System.out.println("Random Gaussian - " + Float.toString(r._b));
		//System.out.println("Square Root of phillips" + Float.toString(phillips(n_prime, m_prime)));
		//phillips is zero
		...
		for (int m_prime1 = 0; m_prime1 < N; m_prime1++) {
			for (int n_prime1 = 0; n_prime1 < N; n_prime1++) {
				...
				if (n_prime == 0 && m_prime == 0) {
					...
					_vertices[index1 + N + Nplus1 * N]._x = _vertices[index1 + N + Nplus1 * N]._ox
							+ h_tilde_dx[index]._a * lambda;
					_vertices[index1 + N + Nplus1 * N]._z = _vertices[index1 + N + Nplus1 * N]._oz
							+ h_tilde_dz[index]._a * lambda;
				...
				if (m_prime == 0) {
					...
					_vertices[index1 + Nplus1 * N]._x = _vertices[index1 + Nplus1 * N]._ox
							+ h_tilde_dx[index]._a * lambda;
					_vertices[index1 + Nplus1 * N]._z = _vertices[index1 + Nplus1 * N]._oz
							+ h_tilde_dz[index]._a * lambda;
